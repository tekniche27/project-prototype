...
export default function Category() {
  ...
  const Button = (props: any) => {
    ...
    const handleClickButton = (name: any, categoryId: any) => {
      setActive(true);
      setColors(name);
    ...
    return (
      <button
        ...
        onClick={() => handleClickButton(props.name, props.categoryId)}
      >
        {props.categoryDesc}
      </button>
    );
  ...
  return (
    <main>
      <div className="grid grid-cols-6 gap-2 justify-center content-center place-content-center place-items-center mt-60">
        {data.map((item: { categoryId: any, categoryCode: any, categoryDesc: any }) => {
          return (
            <Button key={item.categoryId} name="green" categoryDesc={item.categoryDesc} categoryId={item.categoryId}  />
          )
        })}
      </div>
